...
describe('Crud operations test', () => {
    ...
    it('Create the table', async () => {
        const { Info } = await Crud.createTable();
        assert.equal(Info?.TableDescription?.TableName, 'Crud');
    });
    ...
    it('Scan and check that instances are present', async () => {
        const { Items, Errors } = await Crud.scan();
        assert(!Errors);
        assert.deepStrictEqual(Items?.map(i => ({ ...i })), [{ ...crud1 }, { ...crud2 }]);
    });
    it('Delete both instances', async () => {
        const { Info, Errors, Items } = await Crud.select({ crd: [111, 222] }).delete();
        assert(!Errors);
    ...
    it('Update and instance', async () => {
        const { Info, Errors } = await Crud.select({ crd: 111 }).update({
            c: {
                x: ListAppend(false),
                y: {
                    z: AddToSet('asnaeb')
    ...
    it('Use get method and check for successful SET', async () => {
        const { Items, Errors } = await Crud.select({ crd: 111 }).get();
        assert(!Errors);
        assert.deepStrictEqual(Items?.[0], Crud.make({
            a: 'crd',
            b: 111,
            c: {
                x: [true, false],
